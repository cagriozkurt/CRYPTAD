#!/bin/bash
# 01_run_metad_S4_CD2AP_SH3-2.sh  —  CRYPTAD
# WTMetaD for S4 CD2AP SH3-2 domain
# Partition: orfoz (56 cores per node, PLUMED-patched GROMACS 2023.3)
# NOTE: S4 is a small domain (~60 residues) — 56 cores may be over-provisioned.
#   If orfoz is congested, this job can run on hamsi with --ntasks=28 instead.
#
...
#          09_scripts/03_metadynamics/01_run_metad_S4_CD2AP_SH3-2.sh
#
#SBATCH --job-name=meta_S4_SH32
#SBATCH -p orfoz
#SBATCH -A mozkurt
#SBATCH --nodes=1
#SBATCH --ntasks=56
...
RUNDIR=$CRYPTAD/02_md_simulations/CD2AP/metadynamics/S4_SH3-2

RUN1=$CRYPTAD/02_md_inputs/S4_CD2AP_SH3-2/run1
GRO=$RUN1/npt.gro
CPT=$RUN1/npt.cpt
...
TOP_CANDIDATES=($CRYPTAD/02_md_inputs/S4_CD2AP_SH3-2/charmm-gui-*/gromacs/topol.top)
NDX_CANDIDATES=($CRYPTAD/02_md_inputs/S4_CD2AP_SH3-2/charmm-gui-*/gromacs/index.ndx)
...
if [[ -z "$TOP" || ! -f "$TOP" || -z "$NDX" || ! -f "$NDX" ]]; then
    ...
    echo "  Searched: $CRYPTAD/02_md_inputs/S4_CD2AP_SH3-2/charmm-gui-*/gromacs/"
    exit 1
fi

MDP=$SCRIPTS/meta.mdp
PLUMED_DAT=$SCRIPTS/plumed_S4_CD2AP_SH3-2.dat
