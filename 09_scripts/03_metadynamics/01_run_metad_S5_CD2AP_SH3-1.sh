#!/bin/bash
# 01_run_metad_S5_CD2AP_SH3-1.sh  —  CRYPTAD
# WTMetaD for S5 CD2AP SH3-1 domain
# Partition: orfoz (56 cores per node, PLUMED-patched GROMACS 2023.3)
#
# RECOMMENDED FIRST TEST: submit this script for a 1 ns test run first
#   (edit meta.mdp: nsteps=250000 for 1 ns) before full 200 ns runs.
#   After 1 ns: check that HILLS and COLVAR files exist and have >100 lines.
#
...
#          09_scripts/03_metadynamics/01_run_metad_S5_CD2AP_SH3-1.sh
#
#SBATCH --job-name=meta_S5_SH31
#SBATCH -p orfoz
#SBATCH -A mozkurt
#SBATCH --nodes=1
#SBATCH --ntasks=56
...
RUNDIR=$CRYPTAD/02_md_simulations/CD2AP/metadynamics/S5_SH3-1

RUN1=$CRYPTAD/02_md_inputs/S5_CD2AP_SH3-1/run1
GRO=$RUN1/npt.gro
CPT=$RUN1/npt.cpt
...
TOP_CANDIDATES=($CRYPTAD/02_md_inputs/S5_CD2AP_SH3-1/charmm-gui-*/gromacs/topol.top)
NDX_CANDIDATES=($CRYPTAD/02_md_inputs/S5_CD2AP_SH3-1/charmm-gui-*/gromacs/index.ndx)
...
if [[ -z "$TOP" || ! -f "$TOP" || -z "$NDX" || ! -f "$NDX" ]]; then
    ...
    echo "  Searched: $CRYPTAD/02_md_inputs/S5_CD2AP_SH3-1/charmm-gui-*/gromacs/"
    exit 1
fi

MDP=$SCRIPTS/meta.mdp
PLUMED_DAT=$SCRIPTS/plumed_S5_CD2AP_SH3-1.dat
