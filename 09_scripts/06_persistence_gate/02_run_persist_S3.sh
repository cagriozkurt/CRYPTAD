#!/bin/bash
# run_persist_S3.sh  —  CRYPTAD
# SLURM array job: 3 × 20 ns unbiased persistence gate for S3 PICALM ANTH site473
#
# Prerequisites:
...
#
# Output:
#   02_md_simulations/PICALM/persistence_gate/rep{1,2,3}/persist.xtc
#
#SBATCH --job-name=persist_S3
#SBATCH -p hamsi
#SBATCH -A mozkurt
#SBATCH --nodes=1
#SBATCH --ntasks=28
...
#SBATCH --output=slurm_persist_S3_%A_%a.out
#SBATCH --error=slurm_persist_S3_%A_%a.err
...
START=$GATEDIR/site473_start.gro
...
if [[ ! -f "$START" ]]; then
    ...
    echo "       Run 01_extract_persist_start_S3.sh on login node first."
    exit 1
fi
if [[ -z "$TOP" || -z "$NDX" ]]; then
...
echo "=== S3 PICALM ANTH — Persistence gate replica ${REP} ==="
echo "REPDIR : $REPDIR"
echo "START  : $START"
echo "SEED   : $SEED"
echo "Start  : $(date)"
...
cat > "$REPDIR/persist_manifest_rep${REP}.json" <<EOF
{
  ...
  "script": "09_scripts/06_persistence_gate/02_run_persist_S3.sh",
